...
def train_step(images, generator, discriminator, generator_optimizer, discriminator_optimizer, noise):
    #noise = tf.random.normal([BATCH_SIZE, NOISE_DIM]) # No longer needed here
...
def train(dataset, epochs):
    ...
    output_dir = os.path.join('Generierte_Bilder_Farbig', 
                              f'Fixed_Noise_Vector_{timestamp}_Epochen_{EPOCHS}_ImageSize_{IMAGE_SIZE}_NoiseDim_{NOISE_DIM}')
    os.makedirs(output_dir, exist_ok=True)

    #num_images = 50 #No longer used
...
dataset = tf.data.Dataset.from_tensor_slices(images).shuffle(len(images)).batch(BATCH_SIZE) #Keep batch size
