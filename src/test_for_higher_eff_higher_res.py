...
# --- CRITICAL: Ensure image_size is consistently used ---
# Get absolute path to the "Bilder" directory
script_dir = os.path.dirname(os.path.abspath(__file__))
image_dir = os.path.abspath(os.path.join(script_dir, "..", "Bilder"))
image_size = (176, 176)  # Define image_size here
...
def load_and_preprocess_image(path, image_size=(IMG_WIDTH, IMG_HEIGHT)):
...
def create_dataset(image_dir, batch_size=32, image_size=(IMG_WIDTH, IMG_HEIGHT)):
...
def make_generator():
    model = keras.Sequential([
        ...
        # --- Add Resize Layer ---
        keras.layers.Resizing(IMG_WIDTH, IMG_HEIGHT)  # Explicitly resize to 176x176
    ])
    return model
...
def make_discriminator():
    model = keras.Sequential([
        keras.layers.Conv2D(64, (5, 5), strides=(2, 2), padding='same', input_shape=[IMG_WIDTH, IMG_HEIGHT, 1]),
...
generated_image = generated_image.reshape(IMG_WIDTH, IMG_HEIGHT)
