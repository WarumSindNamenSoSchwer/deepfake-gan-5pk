...
# Dynamischer Generator, der sich an die gewünschte Auflösung anpasst
def make_generator(gewollte_res, noise_dim=100):
...
# Dynamischer Diskriminator, der sich an die gewünschte Auflösung anpasst
...
def train_step(images, generator, discriminator, generator_optimizer, discriminator_optimizer):
    ...
    with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
        ...
        real_images = tf.expand_dims(images, -1)  # Bilder haben die Form (gewollte_res, gewollte_res) -> (gewollte_res, gewollte_res, 1)
...
def train(dataset, epochs, project_dir, checkpoint_dir, initial_epoch=0):
    ...
    # Erstelle Modelle von Grund auf
    generator = make_generator(gewollte_res, noise_dim)
